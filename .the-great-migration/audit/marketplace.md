...
2. **`_processPublisherSalesSummary`** — Scans all paid orders (`paymentStatus == 0`) and creates/links `PublisherSalesSummary` records keyed by publisher catalog + quarter. Links orders via `r_publisherToCommerceOrder_c_publisherSalesSummaryId`.
3. **`_processOnHoldTrials`** — SOLUTIONS7 orders in "On Hold" status. Checks seat availability via `GET /trial/availability` and provisions when seats free up via `POST /trial/provisioning`.
4. **`_processPendingOrders`** — Pending orders excluding AI_HUB, DXP, SOLUTIONS7. Auto-completes free orders (Pending → Processing → Completed). Flags paid orders for manual review.
5. **`_processProjectsUsingMarketplaceApps`** — Aggregates orders since 2025-01-01 from non-Liferay users. Queries Koroneiki for associated projects via `GET /koroneiki/contact/by-email-address/{email}`. Stores output in `Report` with id `PROJECTS-USING-MARKETPLACE`.
6. **`_processLiferayStaffUserGroups`** — Assigns "Liferay Staff" role and "SSA-ACCOUNT" account to employees (queries user group "Employees").
7. **`_processRequestProductFeedback`** — CMP_BETA orders created 7–14 days prior (rolling 6-hour windows). Calls `POST /marketplace/request-product-feedback/{orderId}`.
...
2. **Custom field lifecycle** — Orders store JSON in custom fields (trial dates, provisioning configs). Schema changes break running instances.
3. **Cron interdependencies** — Multiple cron jobs read/write the same orders. Any downtime cascades; no alerting visible beyond logs.
4. **Koroneiki project resolution** — Trial provisioning depends on email → Koroneiki contact → teams. Failures silently skip orders.
5. **Portal instance cleanup** — Trial expiry deletes portal instances with no soft-delete or audit trail. Recovery needs cloud backups.
6. **Salesforce sync reliability** — Async via GCF, best-effort. Failures logged, not retried. Missed opportunities unreconcilable.
7. **License key issuance** — Orders sit in Processing if Provisioning is slow/down. No timeout or escalation.
8. **Multi-tenancy** — All objects company-scoped. Consolidation must clarify company strategy.
9. **Notification template names** — Hard-coded strings (`MARKETPLACE-PRODUCT-SUBMIT-TEMPLATE`). Typos silently fail.
10. **Published/draft state** — No visible workflow on PublisherAssets before provisioning; only `processed` flag on PublisherAssetAttachment.
11. **Payment status mapping** — Constants hard-coded in Java (0 = completed, 3 = failed, etc.), implicit sync with Liferay Commerce.
12. **Analytics provisioning** — Free DXP orders auto-provision Analytics workspaces; cost/quota implications not visible.
13. **Timezone handling** — Trial dates use `ZonedDateTime.now()` (system tz) but stored as ISO. Cron runs in system tz, queries use UTC. Potential off-by-one-day bugs.

...
1. **Who is "Provisioning" in the Marketplace's `ProvisioningService` call?** Is it osb-provisioning (which has no REST API in the audited source) or a sibling license-key service (`Provisioning_LicenseKey`'s 230K rows suggest there's real code somewhere)?
2. **Who bridges RabbitMQ (`koroneiki.*`) to Google Pub/Sub (`koroneiki.*`)?** The topic names match — is there a replicator service, or does Koroneiki publish to both buses directly?
3. **How does Support's `KoroneikiAccount` sync run?** The Support codebase has no scheduler or subscriber for this — must be external (cron + REST? message subscriber in a different deployment?). Track it down before assuming the sync dissolves during consolidation.
4. **Is Dossiera being retired?** If yes, Salesforce → new workspace needs a replacement pipeline.
5. **Is LCS being retired?** If yes, Provisioning's `syncToLCS` step disappears.
6. **What's the fate of the legacy `OSB_*` and `Marketplace_App/Module` tables in `prov`?** If they're dead data, archive and drop.
7. **Which of Koroneiki's 62 `EntitlementDefinition.definition` raw-SQL rules are still active and business-critical?** This is the biggest single migration risk — extract them all and review with the product team.
8. **Does the 17M-row `Koroneiki_AuditEntry` need to port forward, or can it be archived?** It will overwhelm a new Object audit log otherwise.

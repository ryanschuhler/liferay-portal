...
2. **`scheduledDeleteTicketAttachment()`** — Hourly (`0 0 * * * *`). Deletes trashed (`WorkflowConstants.STATUS_IN_TRASH`) attachments from GCS.
3. **`scheduledUpdateTicketAttachmentDraftCommentBody()`** — Every hour (`0 0 */1 * * ?`). Retries posting draft comments to Jira for attachments with unsent comments. Condition: `draftCommentBody ne null and state eq 0 and status/any(s:s eq 0)`.
4. **`JiraService.scheduledAffectedVersionsCacheEviction()`** — Admin-triggered via `DELETE /jira/cache`. Clears affected-versions cache.
5. **`JiraService.scheduledIssuesCacheEviction()`** — Admin-triggered. Clears issues cache.
...
2. **RaySource** — `/services/raysource` purpose not clear from code; likely custom analytics or data service.
3. **Jira projects FLS vs HC** — unclear if both actively used or legacy split.
4. **Draft comment retry** — hourly retry of failed Jira comments. No dead-letter queue or customer alerting.
5. **GCS service account key** — injected as `_gcsServiceAccountKey`. Ensure rotation.
6. **Koroneiki sync mechanism undefined** — `KoroneikiAccount` is account-restricted but how/when data flows from Koroneiki is not visible. External process?
7. **Account-scoped RBAC** — many objects use `accountEntryRestrictedObjectFieldName`. Ensure multi-tenant isolation.
8. **No workflow XML found** — state machines (e.g., ticket attachment DRAFT → APPROVED) exist in Spring Boot services / object actions, not WCM workflow definitions.
9. **Notification templates triggering** — 8 templates defined but usage/triggering logic partially visible (likely in Object Actions + scheduled tasks).
10. **Security vulnerabilities read-only** — `/security-vulnerabilities` pulls Jira Security project. Confirm Jira side not writable by customers.
11. **Cache eviction is manual** — no TTL visible on Jira caches; only admin endpoints to clear.
12. **Production credentials** — Jira API key, GCS SA, email creds env-injected. Verify no logging leaks.

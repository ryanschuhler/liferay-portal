...
import {IStructuredContent} from '~/utils/types';
...
const Commerce = () => {
	...
	const fetchCommerceActivationsKeysInstructions = async () => {
		...
		const siteGroupId = Liferay.ThemeDisplay.getSiteGroupId().toString();

		const structuredContentFolders = await fetchHeadless<
			IHeadlessItems<{id: string; name: string}>
		>({
		...
		const structuredContents = await fetchHeadless<
			IHeadlessItems<IStructuredContent>
		>({
			url: `/structured-content-folders/${commerceActivationInstructionsFolderID}/structured-contents`,
		});

		const renderedInstructionsData =
			(await structuredContents?.items.reduce(
				async (
					...
					if (structuredComponent) {
						promiseStructuredContentList.push({
							...
							version:
								(
									dxpVersion as IStructuredContent['contentFields'][0]

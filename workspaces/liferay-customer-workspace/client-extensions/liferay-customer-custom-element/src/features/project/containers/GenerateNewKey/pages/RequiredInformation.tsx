...
interface RequiredInformationValues {
	description: string;
	keys: Key[];
...
interface RequiredInformationProps {
	accountKey: string;
	errors: FormikErrors<RequiredInformationValues>;
	...
	setErrors: React.Dispatch<
		React.SetStateAction<FormikErrors<RequiredInformationValues>>
	>;
	setStep: React.Dispatch<
		React.SetStateAction<(typeof STEP_TYPES)[keyof typeof STEP_TYPES]>
	>;
	setTouched: React.Dispatch<
		React.SetStateAction<FormikTouched<RequiredInformationValues>>
	>;
	state: IGenerateNewKeyState;
	submitKeyAction: ISubmitKeyAction;
	touched: FormikTouched<RequiredInformationValues>;
	urlPreviousPage: string;
	values: RequiredInformationValues;
}
			...
			if (selectedKeyData.hasNotPermanentLicense) {
				...
				if (provisioningService) {
					...
					if (checkedBoxSubscription && isComplimentaryKey) {
						await saveSubscriptionKey(
							(results[0] as any)?.items?.[0]?.id as string
						);
					}
				}
...
const RequiredInformationForm = (props: RequiredInformationFormProps) => {

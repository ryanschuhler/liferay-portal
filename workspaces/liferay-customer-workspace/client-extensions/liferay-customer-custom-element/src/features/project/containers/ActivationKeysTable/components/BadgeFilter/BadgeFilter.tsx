...
import {IDateFilterValue, IFilters, IKeyTypeFilterValue} from '~/utils/types';
	...
	const getDatesDisplay = useCallback(
		(dateFilterState: IDateFilterState): string => {
			...
			if (dateFilterState.value) {
				if (dateFilterState.value.onOrAfter instanceof Date) {
				...
				if (dateFilterState.value.onOrBefore instanceof Date) {

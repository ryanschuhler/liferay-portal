...
public class TicketAttachmentsRestControllerScheduledTest {
	...
	public void setUp() {
		...
		_ticketAttachmentsRestController = new TicketAttachmentsRestController();
		...
		ReflectionTestUtils.setField(
			_ticketAttachmentsRestController, "_liferayOAuth2AccessTokenManager",
		...
		Mockito.when(
			_liferayOAuth2AccessTokenManager.getAuthorization(Mockito.anyString())
		).thenReturn(
			"Bearer test-token"
		);
	}

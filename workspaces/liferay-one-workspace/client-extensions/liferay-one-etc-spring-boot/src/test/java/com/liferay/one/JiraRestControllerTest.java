...
public class JiraRestControllerTest {
	...
	public void testDeleteAccountsBusinessEvents() throws Exception {
		...
		Mockito.verify(
			...
			Mockito.eq("ACCNT-001"), Mockito.eq(ActionKeys.UPDATE),
			Mockito.any()
		);
	...
	public void testGetAccountsTickets() throws Exception {
		...
		Mockito.when(
			_jiraService.getSupportIssues(
				Mockito.eq("ACCNT-001"), Mockito.any())
		).thenReturn(
			List.of()
		);
	...
	public void testPostAccountsBusinessEvents() throws Exception {
		...
		// user-lookup failure through the error handler. The unresolved account
		// raises a PrincipalException, which OneBaseRestController maps to 403.
		...
		_mockMvc.perform(
			MockMvcRequestBuilders.post(
				...
				new JwtAuthenticationToken(TestJwtArgumentResolver.newJwt())
			)
		).andExpect(
			MockMvcResultMatchers.status(
			).isForbidden()
	...
	public void testPutAccountsBusinessEvents() throws Exception {
		...
		// The unresolved user account raises a PrincipalException, which
		// OneBaseRestController maps to 403.

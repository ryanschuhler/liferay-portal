...
| Unit — Spring controller (JUnit + MockMvc) | `./gradlew :client-extensions:liferay-one-etc-spring-boot:test` | No |
| Integration (Playwright `request`) | `yarn test:integration` | Yes |
| E2E (Playwright browser) | `yarn test:e2e` | Yes |

Run only the tier(s) relevant to the change. After editing a Spring controller, run its JUnit test; after editing the custom element, run Vitest; touching either and want end-to-end proof, run the matching Playwright project.
...
# Playwright — one spec
(cd tests && yarn playwright integration/specs/productFeedback.spec.ts)
```

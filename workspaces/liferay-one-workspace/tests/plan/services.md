...
| --- | --- | --- | --- | --- | --- |
| SVC-ACCOUNTSERVICE | Thin headless-admin-user client wrapper — fetches an account by id or ERC; its only branch (not-found → null) cannot be isolated from the static AccountResource client without static mocking | unit | P2 | n/a | service:AccountService |
...
| SVC-COMMERCEORDERSERVICE | Tax-applicability rules for calculate-tax — a business account is taxed only when billed to Ireland, a person account anywhere in the European country set, all other cases untaxed | unit | P1 | planned | service:CommerceOrderService |
...
| SVC-USERACCOUNTSERVICE | Thin headless-admin-user client wrapper — fetches the current or a specified user account with no branch worth proving | unit | P2 | n/a | service:UserAccountService |

...
| Unit | JUnit 5 + Spring MockMvc | Colocated with source in `liferay-one-etc-spring-boot`: `src/test/java/**/*Test.java` | Spring controller logic — routes, status codes, response bodies. No portal required. |
| Integration | Playwright `request` (API only, no browser) | `tests/integration` | Anything that depends on a booted portal: headless REST, the `/o/one/v1` Spring Boot endpoints, OAuth2 scope enforcement, Object definitions. |
...
Java unit tests live under `src/test/java` inside `liferay-one-etc-spring-boot` and use JUnit 5 with Spring's `MockMvc` for controller-layer testing. They do not require a running portal or database.
...
```bash
yarn playwright integration/specs/myUserAccount.spec.ts
```
